...
void pmem_init(unsigned int mbi_addr)
{
    ...
    else
    {
        ...
        nps = (start_address / PAGESIZE) + (length / PAGESIZE) + (start_address % PAGESIZE + length % PAGESIZE) / PAGESIZE;
    ...
    {
        start_address = get_mms(i);
        length = get_mml(i);
        perm = is_usable(i);
        if (perm == 0) // means page is reserved
        {
            perm = 0; // reserved by BIOS
        }
        else
        {
            perm = 2; // perm>1 means page is useable
        }

        page_index = start_address / PAGESIZE;

        // allign page index to the beginning of the page

        if (page_index * PAGESIZE < start_address)
        {
            page_index++;
        }

        for (; (page_index + 1) * PAGESIZE <= (start_address + length); page_index++)
        {
            if (page_index < VM_USERLO_PI)
            {
                continue;
            }
            if (page_index >= VM_USERHI_PI)
            {
                break;
            }
